...
const char *command_helptext[COMMAND_COUNT << 1] = {
	...
	"steps the machine in parts or full steps (full if not specified), number of steps defaults to 1 if not specified",
	"run [<part|full>,<delay>]",
	"runs the machine in partial or full steps (full if not specified) with an optional delay in milliseconds",
	"poke <location^>,<value>",
	...
	"reset",
	"resets machine memory and registers",
	"help",
...
const char *location_helptext[SPECIAL_LOC_COUNT << 1] = {
	...
	"(program_counter)",
	"ir",
	"(instruction register) - poking may lead to undefined behavior",
	"sr",
	"(status register) - poking may lead to undefined behavior"
};
...
u8 run_command(rr_machine_t *machine, char *cmd, char **operands) {
	...
	if(!strcmp(cmd, "save")) {
		...
		switch(machine_save(machine, operands[0])) {
			...
			case 1:
				fprintf(stdout, "Could not open file for saving\n");
				break;
			
			case 2:
				fprintf(stdout, "Could not write to file\n");
				break;
		...
		switch(machine_load(machine, (const char *)operands[0])) {
			...
			case 1:
				fprintf(stdout, "Could not open file for loading\n");
				break;
			
			case 2:
				fprintf(stdout, "Could not read from file\n");
				break;
		...
		fprintf(stdout, "Unrecognized command entered. List of commands:\n");
